...
def on_button_click():
    ...
    if version_choice == 1:
        rotas = pokemons_rotas_fire_red
    elif version_choice == 2:
        rotas = pokemons_rotas_leaf_green
    else:
        messagebox.showerror("Erro", "Escolha de versão inválida.")
        return

    incluir_inicial = incluir_inicial_var.get()
    time_aleatorio = criar_time(incluir_inicial, rotas)
...
# Widgets
label_version = tk.Label(root, text="Escolha a versão:")
radio_fire_red = tk.Radiobutton(root, text="Fire Red", variable=version_var, value=1)
radio_leaf_green = tk.Radiobutton(root, text="Leaf Green", variable=version_var, value=2)
label_incluir_inicial = tk.Label(root, text="Incluir inicial no time?")
checkbutton_incluir_inicial = tk.Checkbutton(root, variable=incluir_inicial_var)
button_generate_team = tk.Button(root, text="Gerar Time Aleatório", command=on_button_click)
label_result = tk.Label(root, textvariable=result_text)

# Layout
label_version.grid(row=0, column=0, columnspan=2)
radio_fire_red.grid(row=1, column=0)
radio_leaf_green.grid(row=1, column=1)
label_incluir_inicial.grid(row=2, column=0, columnspan=2)
checkbutton_incluir_inicial.grid(row=3, column=0, columnspan=2)
